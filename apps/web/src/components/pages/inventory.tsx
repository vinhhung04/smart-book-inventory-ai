  ...
  const warehouses = useMemo(() => {
    ...
    for (const item of data) {
      for (const location of item.locations || []) {
        set.add(location.warehouse_name);
      }
    }
    return ["All Warehouses", ...Array.from(set).sort((a, b) => a.localeCompare(b))];
  }, [data]);

  const filtered = data
    .filter((item) => {
      const status = getStockStatus(Number(item.quantity || 0));

      if (whFilter !== "All Warehouses") {
        const hasWarehouse = (item.locations || []).some((location) => location.warehouse_name === whFilter);
    ...
    .filter((item) => {
      ...
      return item.title.toLowerCase().includes(keyword) || String(item.isbn || "").toLowerCase().includes(keyword);
    });

  const totalUnits = data.reduce((sum, item) => sum + Number(item.quantity || 0), 0);
  const healthyCount = data.filter((item) => getStockStatus(Number(item.quantity || 0)) === "in-stock").length;
  const lowCount = data.filter((item) => getStockStatus(Number(item.quantity || 0)) === "low-stock").length;
  const outCount = data.filter((item) => getStockStatus(Number(item.quantity || 0)) === "out-of-stock").length;
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[12px] text-slate-400 mt-0.5">{data.length} items · {totalUnits} total units</p>
            </div>
          </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100 bg-gradient-to-r from-emerald-50/40 to-transparent">
                {["Barcode", "Title", "Category", "Location", "Qty", "Health", "Status", "Updated"].map(h => (
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={8} className="text-center py-14 text-[13px] text-slate-400">Dang tai du lieu ton kho...</td></tr>
              ) : filtered.length === 0 ? (
                ...
                return (
                  <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                    ...
                    <td className="px-5 py-3.5 text-[12px] font-mono text-slate-400">{item.isbn || "-"}</td>
                    ...
                    <td className="px-5 py-3.5 text-[12px] font-mono text-slate-500">{item.location || "-"}</td>
                    ...
                    <td className="px-5 py-3.5 text-[12px] text-slate-400">{formatUpdatedTime(item.updated_at)}</td>
                  </motion.tr>
                );
              })}
            </tbody>
